...
def remove_stop_words(text):
    stop_words = ['devi', 'dei', 'debi','kumar','singh', 'kmr', 'kumr','bhai', 'bhau', 'bai', 'ben', 'kaur', 'Md', 'Mohd', 'Mohammad', 'Mohamad','alam','shekh','sek']
    words = text.split()
...
def process_false_cases(row):
    ...
    fuzzy_flag = 1 if fuzzy_SS >= 0.65 else 0
...
def process_name_matching_from_names(name1, name2):
    ...
    df_layer1 = pd.DataFrame(data)
    
    df["Prediction"] = 0 
    
    df[["First_Layer_Score", "first_layer_pass"]] = df.apply(
    ...
    df["Prediction"] = df["first_layer_pass"].astype(int)  
    ...
    df["Third_Layer_Score"] = df.apply(process_false_cases, axis=1)
    df["Prediction"] = 1 if df["Third_Layer_Score"].iloc[0] else 0
